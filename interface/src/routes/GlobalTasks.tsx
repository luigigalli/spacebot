...
import { Button, Popover, SelectPill, OptionList, OptionListItem } from "@spacedrive/primitives";
import {
  TaskList,
  TaskDetail,
  TaskCreateForm,
...
import { GithubMetadataBadges, getGithubReferences } from "@/components/TaskUtils";
    ...
    <div className="flex items-center gap-2">
      ...
      <Popover.Root open={open} onOpenChange={setOpen}>
        <Popover.Trigger asChild>
          <SelectPill size="sm">{selected?.display_name ?? selected?.id ?? "Select agent"}</SelectPill>
        </Popover.Trigger>
...
export function GlobalTasks() {
  ...
  const [collapsedGroups, setCollapsedGroups] = useState<Set<UiTaskStatus>>(() => new Set());
  ...
  const updateMutation = useMutation({
    mutationFn: ({ taskNumber, ...req }: { taskNumber: number; status?: TaskStatus; complete_subtask?: number }) =>
    ...
    <div className="flex h-full w-full">
      ...
      {activeTask && (
        <div className="w-[400px] shrink-0 overflow-y-auto border-l border-app-line">
          ...
          />
          <GithubSection metadata={(activeTask as unknown as TaskItem).metadata} />
        </div>
      )}
    </div>
  );
